...
const Deliveryprofile = () => {
  const getTokenFromLocalStorage = localStorage.getItem("delivery")
  ...
  const userState = useSelector((state) => state.delivery.delivery);
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      dispatch(updateDelivery({ data: values, config2: config2 }));
      setEdit(true);
    },
  });
